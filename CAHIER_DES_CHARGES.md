...
| **Agent marketing** | **Nouveau, 2026-08-02** | Bras terrain du Marketing/Responsable Marketing : campagnes, prospection, transforme les prospects en clients. Accès : Tableau de bord, Marketing, Clients — rien d'autre (pas d'Affaires/Trésorerie, comme Freelance/Commercial). Recrutement définitif ou temporaire (une campagne, ex. un mois) : même registre RH que les autres postes (`personnel.type_contrat` SALARIE ou PARTENAIRE selon le cas), aucune distinction technique supplémentaire nécessaire. Créé par anticipation ("dès qu'on a un agent, l'application sait comment le gérer") — personne n'occupe ce rôle pour l'instant, mais le poste est prêt. |
...
~~**Incohérence relevée, à trancher**~~ — **résolu 2026-08-02** : "Agent marketing" créé comme rôle à part entière (ci-dessus), pas fondu dans Employé. "Marketing" (responsable) reste, pour l'instant, accessible uniquement via Admin/Super Admin — l'utilisateur n'a pas demandé de rôle "Responsable Marketing" séparé, seulement l'agent de terrain ; à rouvrir s'il en a besoin plus tard.
...
**Marketing — scope délégué entièrement à Claude par l'utilisateur (2026-08-02, "tu gères"), implémenté.** L'utilisateur a explicitement demandé à Claude d'utiliser sa propre connaissance de ce qu'un module marketing doit apporter à une boutique/des produits, plutôt que de deviner un minimum. Construit :
- **Promotions** (table `promotions`) : réduction en pourcentage ou montant fixe sur un article publié en boutique, avec fenêtre de dates. **Le prix réel du Catalogue (`articles.prix_vente`) n'est jamais modifié** — la promotion est une couche d'affichage/calcul temporaire et réversible, appliquée uniquement à l'affichage `/boutique` (prix barré + badge de réduction). Une promotion n'est "active" que si elle est `actif=true` ET dans sa fenêtre de dates.
- **Bannière d'annonce** (`parametres_marketing`, singleton) : message affiché en haut de `/boutique` (soldes, nouveauté, annonce saisonnière), activable/désactivable.
- Écran `/marketing` (Admin/Super Admin uniquement — module déjà présent dans la matrice §6/§7, personne d'autre ne l'a actuellement).
- "Demandes de devis → Affaires" (l'autre moitié de la spec §7) : déjà couverte par le module Commercial existant (§12, `/commercial`, PROFORMA) — pas de nouvelle logique nécessaire, juste réutilisée.

Testé de bout en bout sur Neon : promotion -20% sur Polo Standard (9 000 F → 7 200 F) créée dans `/marketing` → affichée correctement sur `/boutique` avec prix barré et badge ; bannière activée → affichée en haut de `/boutique`.

**Achats, Dépenses, Charges — clarifiés par l'utilisateur (2026-08-02) et implémentés.** Poussé par l'utilisateur qui a fait remarquer, à raison, que la complexité (fournisseurs, articles, lots, sorties/entrées d'argent — loyer, courant, transport, imprévus) existe déjà réellement dans l'app : Fournisseurs+Stock (lots) pour les achats, `bons_decaissement` (Trésorerie, catégories ACHAT_MARCHANDISE/CHARGE_GENERAL, avec le seuil de validation hiérarchique §16.7 déjà en place) pour l'argent qui sort. Le vrai manque n'était pas la logique mais des **écrans dédiés** rendant cette réalité visible d'un coup d'œil, plutôt qu'éparpillée entre `/fournisseurs`, `/stocks` et `/tresorerie`.
- **`/achats`** : statistiques (fournisseurs actifs, lots réceptionnés, total achats marchandise validés du mois), liste des derniers lots (article, fournisseur, quantité, prix d'achat), liste des sorties de caisse ACHAT_MARCHANDISE, formulaire de création rapide (réutilise `creerBonDecaissement`, aucune logique dupliquée). Lien direct vers `/fournisseurs` pour la gestion complète des fiches fournisseur (pas de duplication d'écran).
- **`/depenses` et `/charges`** : consolidés en un seul écran — le cahier des charges ne distingue pas assez les deux noms pour justifier deux écrans séparés, tous deux pointent sur la catégorie CHARGE_GENERAL existante. Boutons rapides (Loyer, Courant/électricité, Transport, Imprévu) pré-remplissent le motif ; formulaire de création réutilise `creerBonDecaissement`.

Testé de bout en bout sur Neon : bon d'achat de 5 000 F créé → reflété immédiatement dans le total du mois `/achats` ; bon "Loyer" de 75 000 F créé → correctement marqué "En attente" (au-dessus du seuil de validation hiérarchique de 50 000 F, §16.7) et exclu du total "validées" tant que non validé — même comportement que n'importe quel décaissement classique. `/charges` redirige vers `/depenses` et affiche les mêmes données.
...
**Dimension Finance — implémentée et vérifiée en base réelle (2026-08-02)** : `/rapports` (module déjà présent dans la matrice §6/§7, aucune extension inventée). 5 fréquences sélectionnables (jour/semaine/mois/semestre/année), toujours recalculé à la demande — aucune persistance. CA = somme des `affaires.montant_ttc` statut VALIDEE/CLOTUREE sur la période ; coût d'achat des ventes = Σ quantité × `articles.pmp` (**limitation assumée** : PMP courant, pas un historique figé au moment de la vente — le schéma ne conserve pas de coût gelé par ligne) ; dépenses/charges = décaissements validés (catégories ACHAT_MARCHANDISE/CHARGE_GENERAL) ; commissions = bulletins de paie payés sur la période (§7 RH). Testé de bout en bout sur Neon : 19 600 F de CA, 12 000 F de coût (vérifié contre le PMP réel de l'article = 6 000 F × 2 pièces), 7 600 F de bénéfice brut, 2 910 F de commissions, 4 690 F de bénéfice net — arithmétique confirmée exacte. **Dimensions RH, Incidents et Prévisions — clarifiées par l'utilisateur (2026-08-02) et implémentées.** Ce ne sont PAS des indicateurs financiers : ce sont des réalités humaines, portées par le module RH.
- **RH** : effectif actif + masse salariale payée sur la période.
- **Incidents** : événements touchant le personnel — maladie, blessure, décès, catastrophe naturelle empêchant le travail, blocage de recrutement — déclarés dans `/rh` (table `incidents_personnel`), comptés par type sur la période sélectionnée. Champ `obligationsLegales` en saisie libre uniquement — **aucune règle de droit du travail malien n'est codée en dur ou déduite automatiquement**, l'utilisateur a explicitement demandé "qu'est-ce que la loi dit" mais Claude ne peut pas fiabiliser ça sans vérification juridique réelle.
- **Prévisions** : planification RH des besoins de personnel à venir (renfort saisonnier — ex. saison des pluies —, contrats partenaires, recrutement à anticiper) déclarés dans `/rh` (table `besoins_saisonniers`), pas une prévision de chiffre d'affaires. Un besoin est "actif" sur une période si sa fenêtre `periodeDebut`/`periodeFin` chevauche la période du rapport, pas seulement s'il a été créé dedans.

Testé de bout en bout sur Neon : incident "Maladie" déclaré → compté correctement dans le rapport Incidents du mois ; besoin "Renfort saison pluvieuse 2026" (3 personnes, août-octobre) → apparaît correctement dans le rapport Prévisions du mois.

**RH — implémenté et vérifié en base réelle (2026-08-01)** : `/rh` (module "RH" déjà présent dans la matrice de permissions §6/§7 — Super Admin et Admin uniquement pour l'instant, aucune extension inventée). Registre `personnel` léger (nom, téléphone, fonction, type de contrat SALARIE/JOURNALIER/PARTENAIRE, salaire de base, taux de commission optionnel, `utilisateurId` optionnel pour relier un compte applicatif réel) — volontairement découplé de `utilisateurs` : un Journalier n'a explicitement "aucun compte applicatif" (§6) mais doit quand même apparaître en paie. Bulletins de paie (`bulletins_paie`, un par personnel/période `YYYY-MM`) : salaire de base, prime transport, commission (bouton "Suggérer" — somme des `affaires.montant_ttc` dont ce personnel est l'auteur sur la période × taux, éditable ensuite), retenue INPS, avance, net à payer calculé. "Marquer payé" réutilise `creerBonDecaissement` (catégorie `RH_SALAIRE`, déjà présente dans le schéma) plutôt que de dupliquer la logique de seuil de validation hiérarchique (§16.7) — testé : un bulletin au-dessus du seuil (50 000 F) atterrit bien en "Validation hiérarchique requise" dans `/tresorerie`, exactement comme un décaissement classique. Testé de bout en bout sur Neon : personnel créé → commission suggérée (2 910 F = 29 100 F × 10%, vérifié contre la somme réelle des affaires du mois) → bulletin généré → marqué payé → bon de décaissement RH/Salaire visible dans `/tresorerie`. PDF Fiche de paie construit dans la foulée (§13). Reste hors périmètre : calcul automatique des cotisations INPS (saisie manuelle pour l'instant).

**Incidents et besoins saisonniers — ajoutés le 2026-08-02**, deux onglets supplémentaires de `/rh` : voir le détail sous "Rapport" ci-dessus (dimensions Incidents/Prévisions). Tables `incidents_personnel` et `besoins_saisonniers`, mêmes droits d'accès que le reste du module RH.
...
**Implémenté et vérifié en base réelle (2026-07-29)** : module `/rd-calculateurs` (rôles Admin/Super Admin/Manager/Vendeur, module "R&D"). Bibliothèque de références en base (`encres_marquage`, `supports_marquage`, `cadres_serigraphie`, `paliers_broderie`, `parametres_marquage` — singleton main d'œuvre/marge par défaut), seedée avec les valeurs de la maquette validée, éditable derrière le bouton discret Admin. Moteur de coût pur (`lib/calculateurs/marquage.ts`) partagé, testable indépendamment de la DB. Écran vente : sélection d'un article/variante Famille A réel (Stock), clic-zone + boutons rapides, prix calculé en direct, "Créer l'affaire" produit une `ligne_affaire` normale (nouvelle colonne `config_marquage` en JSON pour audit/référence, jamais recalculée) — indiscernable du reste du système : contrôle de stock, décrément FIFO, ticket, tout fonctionne sans code spécifique. Testé de bout en bout sur Neon : zone Sublimation 10×10cm → encre 164F + support 50F (vérifié à la formule près), prix total exact, affaire validée, stock décrémenté normalement. Écrans Ensemble complet / mode Tissu / configurateur produit-first restent à construire (§10bis, chemin long) — cette passe couvre le moteur de coût par technique, réutilisable par ces écrans plus tard.
...
**Production industrielle de pagne — tranché différemment le 2026-08-02, pour "faire sain" pour un début.** Revient sur la décision du 2026-07-29 (dossier séparé `dossiers_pagne`, jamais implémenté) : plutôt qu'un cycle à part avec acomptes échelonnés et suivi de production dans l'app, le pagne industriel est traité **comme un article normal** — Devis/Proforma → Facture, le circuit standard déjà construit (§4.5, §12), rien de spécifique à coder. Emballage et logistique de production restent gérés **en dehors de l'application** pour l'instant (le fournisseur usine tiers, pas EVOLUTIS223). Seul ajout : un **prix de revient** saisi manuellement sur la fiche article (Famille C ou D — ces familles n'ont pas de lot d'approvisionnement qui alimenterait `articles.pmp` automatiquement comme la Famille A/B, §4.3) — construit et vérifié en base réelle (2026-08-02, `/catalogue`, champ "Prix de revient" à la création et à l'édition, alimente aussi le coût d'achat des ventes dans `/rapports`). Un système plus complet (suivi de production, acomptes échelonnés, spécifications liées) est explicitement envisagé **plus tard**, en évolution, une fois que l'activité aura donné plus de visibilité sur ce segment — pas à deviner maintenant.

**Parcours maquette — implémenté et vérifié en base réelle (2026-07-31)** : route publique `/maquette` (`proxy.ts`, sans connexion), parcours 9 écrans porté depuis la maquette Artifact validée (intent → dépôt de références/guidage → disposition des éléments par nombre réel 3/4/6 issue de `dispositions_maquette` → couleurs → coordonnées → forfait Famille C réel C-001..C-004 → mode de réception → récap → confirmation). Une soumission écrit dans une table dédiée `demandes_maquette` (statut `EN_ATTENTE`, numéro `MAQ-AA-NNNN`) plutôt que directement dans `affaires` — même logique de dossier séparé que le pagne industriel ci-dessus, nécessaire ici aussi car `affaires.auteurId` exige un utilisateur connecté que le client public n'a pas. Uploads de logos/modèles vers Vercel Blob (`lib/blob.ts`, 8 Mo max, JPEG/PNG/WebP/SVG) — nécessite un `BLOB_READ_WRITE_TOKEN` réel côté utilisateur avant mise en production, pas encore fourni. File d'attente intégrée à `/validations` (§9) : "Valider (crée l'affaire)" retrouve/crée le `client` par téléphone et appelle `creerAffaire()` existant → `COMMANDE_ATTENTE` normale, visible et gérable dans `/affaires` sans aucun code spécifique en aval. "Refuser" marque `REFUSEE`. Réglages (bibliothèque de modèles, verrouillage des dispositions par nombre, forme/taille des médaillons) dans une route admin séparée `/maquette-admin` (Admin/Super Admin uniquement, pas de module dédié dans la matrice §6/§7 — même traitement que Fournisseurs/R&D) plutôt qu'un bouton caché sur la page publique comme dans la maquette Artifact, qui n'offrait qu'une protection cosmétique. Testé de bout en bout sur Neon : soumission publique → `MAQ-26-0001` en base → visible dans `/validations` → Valider → `CDE-26-0002` créée dans `/affaires` (25 000 F, client "Client Test Maquette" auto-créé) ; accès `/maquette-admin` confirmé bloqué pour un rôle Vendeur.
...
**Reçu de caisse — premier modèle construit et validé (2026-07-28)** : `lib/documents/recu-caisse.tsx`, généré via `@react-pdf/renderer` (pur JS, voir §16 nouvelles notes techniques). QR code ajouté (`lib/documents/qr.ts`, lib `qrcode` pure JS) — payload provisoire `EVOLUTIS223-SUIVI:{numero}` en attendant la vraie page de suivi (§11) et un nom de domaine.

**Paramétrage des documents — tranché 2026-07-28 : option 1 (paramétrage par champs/sections)**, cohérent avec le principe de paramétrage ci-dessus (§3.1) — pas d'éditeur de gabarit universel, juste des sections/textes basculables par type de document. Table `parametres_documents` (type_document, config JSONB, modifie_par, date_modification) construite et branchée (2026-07-29) sur Reçu de caisse (mentions légales, message de remerciement) et Bon de livraison (mentions légales, libellé de la signature EVOLUTIS223) via `lib/documents/parametres.ts` — pas encore d'écran Admin pour l'éditer (les valeurs par défaut suffisent pour l'instant, l'action `enregistrerParametresDocument` existe déjà pour quand ce sera nécessaire).

**Bon de livraison — deuxième modèle construit et vérifié en base réelle (2026-07-29)** : `lib/documents/bon-livraison.tsx`, servi à la demande (pas de persistance) via `GET /api/documents/bon-livraison/[affaireId]`, lien "Bon de livraison" ajouté sur chaque commande de `/commandes`. Écart assumé vs le mockup source : le libellé de signature droite ("Le Fournisseur", copié-collé depuis le modèle Bon de commande) corrigé en "EVOLUTIS223" (paramétrable). Quantité livrée = quantité commandée seulement si l'affaire est validée/clôturée et, en mode Livraison, la course marquée "Livrée" — sinon 0.

**Fiche de paie — troisième modèle construit et vérifié en base réelle (2026-08-02)**, maintenant que RH (§7) existe : `lib/documents/fiche-paie.tsx`, servi à la demande via `GET /api/documents/fiche-paie/[bulletinId]`, lien "PDF" ajouté sur chaque bulletin de `/rh`. Rubriques (salaire de base, prime transport, commission, retenue INPS, avance) affichées signées (gain/retenue), net à payer en pied de tableau. Pas de QR — contrairement au Bon de livraison, ce document n'a pas de pendant "suivi public" (§11) à encoder. Paramétrage (`FICHE_PAIE`) branché sur `parametres_documents` comme les deux précédents.

**Bon de commande, Ordre de mission, Courrier — pas encore attaqués.** Contrairement au Reçu de caisse, au Bon de livraison et à la Fiche de paie, ces trois documentent des modules qui n'existent pas encore en base (Achats/Fournisseurs — Phase 4, §14) : les construire maintenant produirait un générateur PDF sans données réelles à mapper. À reprendre quand Achats/Fournisseurs sera codé. Le Courrier (corps libre, pas de données métier) pourrait en théorie être fait dès maintenant mais n'a pas de point d'intégration naturel identifié — à clarifier avec l'utilisateur (à qui/quoi sert-il concrètement dans l'appli ?) avant de le construire.
