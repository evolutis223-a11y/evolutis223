...
**Bon de commande, Fiche de paie, Ordre de mission, Courrier — pas encore attaqués.** Contrairement au Reçu de caisse et au Bon de livraison, ces trois premiers documentent des modules qui n'existent pas encore en base (Achats/Fournisseurs, RH — Phase 4, §14) : les construire maintenant produirait un générateur PDF sans données réelles à mapper, même problème que le Reçu de caisse est resté injoignable un temps. À reprendre quand Achats/Fournisseurs et RH seront codés. Le Courrier (corps libre, pas de données métier) pourrait en théorie être fait dès maintenant mais n'a pas de point d'intégration naturel identifié — à clarifier avec l'utilisateur (à qui/quoi sert-il concrètement dans l'appli ?) avant de le construire.
