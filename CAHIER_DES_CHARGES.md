...
8. **Répartition de la réserve au prorata des tailles** — à l'appro, une réserve détail demandée en pièces (ex. 12) se répartit "au prorata de chaque taille" (confirmé par `design/Workflow...`), mais l'algorithme exact d'arrondi (reste de division non nul) n'est pas précisé — à définir avant de coder l'écran d'approvisionnement (§8.1 note UX : le champ se saisit en PIÈCES, pas en douzaines — piège relevé dans le doc source, à afficher clairement dans l'UI).
