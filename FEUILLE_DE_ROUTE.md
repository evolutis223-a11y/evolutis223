...
| 1 — Cœur métier | **80%** | 1.1→1.5 faits et vérifiés en base réelle. 1.5 Commandes : mode Retrait (Prête → retirée → Clôturée) et Livraison (En attente → Pris en charge → En route → Livrée → affaire Clôturée automatiquement) testés de bout en bout. Bug réel trouvé et corrigé en cours de route : les articles Famille B ne résolvaient jamais de variante, donc le contrôle de stock ET le décrément FIFO les ignoraient silencieusement — corrigé avant commit. Génération PDF (§8.4) avancée en parallèle (Reçu de caisse seul, 5 restants). Reste : 1.6 Trésorerie (bons de décaissement, clôture caisse). |
