...
**Dimension Finance — implémentée et vérifiée en base réelle (2026-08-02)** : `/rapports` (module déjà présent dans la matrice §6/§7, aucune extension inventée). 5 fréquences sélectionnables (jour/semaine/mois/semestre/année), toujours recalculé à la demande — aucune persistance. CA = somme des `affaires.montant_ttc` statut VALIDEE/CLOTUREE sur la période ; coût d'achat des ventes = Σ quantité × `articles.pmp` (**limitation assumée** : PMP courant, pas un historique figé au moment de la vente — le schéma ne conserve pas de coût gelé par ligne) ; dépenses/charges = décaissements validés (catégories ACHAT_MARCHANDISE/CHARGE_GENERAL) ; commissions = bulletins de paie payés sur la période (§7 RH). Testé de bout en bout sur Neon : 19 600 F de CA, 12 000 F de coût (vérifié contre le PMP réel de l'article = 6 000 F × 2 pièces), 7 600 F de bénéfice brut, 2 910 F de commissions, 4 690 F de bénéfice net — arithmétique confirmée exacte. **Dimensions RH, Incidents et Prévisions non construites** : le cahier des charges ne précise pas ce qu'elles doivent afficher concrètement (quel indicateur pour "Incidents" ? quel modèle de calcul pour "Prévisions" ?) — deviner une forme aurait été plus risqué que de livrer la seule dimension réellement spécifiée (Finance) et d'attendre une clarification pour les trois autres.
